...
pz = 0.7; %% the probability of zombie successfully converting a human to a zombie
ph = 0.3; %%
...
H = zeros(h0,3);
OB = zeros(ob,2);
% set initial random positions of agents
Z(1:z0,1:2) = lim*rand(z0,2);
H(:,1:2) = lim*rand(h0,2);
for i = 1:30:ob
  OB(i,1:2) = lim*rand(1,2);
  xR = randi([-1 1],1);
  yR = randi([-1 1],1);
  for j = 1:30
    OB(i+j,1) = OB(i+j-1,1) + xR;
    OB(i+j,2) = OB(i+j-1,2) + yR;
  end
end
...
while z>0 && h>0
  ...
  plot(OB(1:ob,1),OB(1:ob,2),'.','MarkerEdgeColor','g','MarkerSize',15);
  xlim([-1 lim]);
  ylim([-1 lim+1]);
  hold off
  pause(framePause);
  ...
  % Hz: index of closer human to each zombie, size equal to z0
  % Zh: index of closer zombie to each human, size equal to h0
